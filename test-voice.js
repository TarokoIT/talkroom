import {installDiagnostics} from './ice-diagnostics.js?v=1';
const diagnostic=installDiagnostics();
...
function attach(call){
 const number=who(call.peer),row={call,number,id:++sequence};all.add(row);active.set(call.peer,call);diagnostic.note('call-binding',{call:row.id,pc:diagnostic.pcId(call.peerConnection),endpoint:number,connection:call.connectionId});
...
$('mic').onclick=()=>{mic=!mic;$('status').textContent='第 '+slot+' 端 · 發話'+(mic?'開啟':'關閉');stream.getAudioTracks().forEach(t=>t.enabled=mic);$('mic').textContent=mic?'停止發話':'開啟發話';log('mic',{enabled:mic});if(mic)setTimeout(dial,300);else{for(const call of active.values())call.close();active.clear();} }; // Original microphone-off lifecycle.
...
$('leave').onclick=()=>leave();
$('export').onclick=()=>{const blob=new Blob([JSON.stringify({version:'TEST 1.2',slot,at:new Date().toISOString(),events,samples,diagnostics:diagnostic.report()},null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='talkroom-test-'+(slot||'unknown')+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
