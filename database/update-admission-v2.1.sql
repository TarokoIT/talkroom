...
DECLARE
 ...
 active_count int; result_id uuid; peers jsonb; msgs jsonb; current_revision bigint; lock_room text;
BEGIN
 ...
 IF p_action='join' THEN
  ...
  PERFORM pg_advisory_xact_lock(hashtext('talkroom:'||coalesce(p_payload->>'room','')));
  ...
  END IF;
  UPDATE talkroom_private.sessions SET offline_logged=true WHERE expires_at<now() AND NOT offline_logged;
 ...
 END IF;
 -- Match room-admin/join lock ordering before locking a reconnecting session.
 IF p_action='sync' THEN
  SELECT room INTO lock_room FROM talkroom_private.sessions WHERE id=p_session AND user_id=uid
   AND last_seen<=now()-interval '60 seconds';
  IF FOUND THEN PERFORM pg_advisory_xact_lock(hashtext('talkroom:'||lock_room)); END IF;
 END IF;
 ...
 IF p_action='sync' THEN
  IF me.last_seen<=now()-interval '60 seconds' THEN
   UPDATE talkroom_private.sessions SET offline_logged=true WHERE id=me.id AND NOT offline_logged;
  ...
  END IF;
  UPDATE talkroom_private.sessions SET last_seen=now(),offline_logged=false,
  ...
  SELECT history_revision INTO current_revision FROM talkroom_private.rooms WHERE code=me.room;
  IF p_payload->>'history_revision'=current_revision::text AND p_payload->>'latest_message_id'=(SELECT id::text FROM talkroom_private.messages WHERE room=me.room ORDER BY created_at DESC,id DESC LIMIT 1) THEN
   msgs:=NULL;
  ELSE
  ...
  END IF;
  RETURN jsonb_build_object('ok',true,'peers',peers,'messages',msgs,'history_revision',current_revision);
 END IF;
