...
BEGIN
 ...
 BEGIN PERFORM public.talkroom_interact(s,'ack');RAISE EXCEPTION 'legacy ack allowed';EXCEPTION WHEN raise_exception THEN IF SQLERRM='legacy ack allowed' THEN RAISE;END IF;END;
 ...
 BEGIN PERFORM public.talkroom_interact(t,'ack');RAISE EXCEPTION 'legacy ack allowed';EXCEPTION WHEN raise_exception THEN IF SQLERRM='legacy ack allowed' THEN RAISE;END IF;END;
