...
Candidate 2: clear unanswered calls after 20 seconds, failed/closed calls immediately on the next health sample, and continuously disconnected calls after an 8-second grace period. Existing 2-second authorized roster polling redials when the local microphone remains enabled. Connected calls are retained. Added local per-call identifiers, direction, playback/mute state and teardown reasons. Three lifecycle decision tests pass; handset validation remains pending.
